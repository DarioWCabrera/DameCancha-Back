...
export class DisponibilidadService {
  ...
  async replaceForCancha(
    ...
    for (const item of normalized) {
      if (!Number.isInteger(Number(item.dia_semana)) || Number(item.dia_semana) < 0 || Number(item.dia_semana) > 6) {
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      await repo.delete({ cancha: { id_cancha: idCancha } });
  ...
  async update(id: number, dto: UpdateDisponibilidadDto) {
    ...
    if (!entity) throw new NotFoundException('Disponibilidad no encontrada.');

    const start = dto.hora_inicio ?? entity.hora_inicio;
    const end = dto.hora_fin ?? entity.hora_fin;
    this.validateRange(start, end);

    if (dto.dia_semana !== undefined) entity.dia_semana = dto.dia_semana;
    if (dto.hora_inicio !== undefined) entity.hora_inicio = dto.hora_inicio;
    if (dto.hora_fin !== undefined) entity.hora_fin = dto.hora_fin;
    if (dto.id_cancha !== undefined) entity.cancha = { id_cancha: dto.id_cancha } as any;
    return this.disponibilidadRepository.save(entity);
  }

  async remove(id: number) {
    const result = await this.disponibilidadRepository.delete({ id_disponibilidad: id });
    if (!result.affected) throw new NotFoundException('Disponibilidad no encontrada.');
    return { message: 'Disponibilidad eliminada correctamente.', id_disponibilidad: id };
  }
}
